 ...
 * Мы добавляем свои поля: displayName, rating, gamesPlayed, gamesWon, theme.
 *
 * ВАЖНО: встроенные маршруты /auth/register и /auth/login
 * заполняют только стандартные колонки. Поэтому все наши NOT NULL поля
 * имеют .default(...), иначе регистрация упадёт с NOT NULL.
 * `theme` — nullable text (light | dark | unset), без NOT NULL.
 */
